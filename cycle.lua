...
cyclefocus.filters = {
  ...
  marked = function(c, _) --luacheck: no unused args
  ...
  not_through_focus_signal = function(c, _) --luacheck: no unused args
...
cyclefocus.get_client_title = function(c, _) --luacheck: no unused args
...
-- @client c
-- @return table
cyclefocus.decorate_show_client_others = function(_) --luacheck: no unused args
  return {
    -- XXX: too distracting.
    -- opacity = 0.7
  }
...
cyclefocus.cycle = function(startdirection_or_args, args)
      ...
      if nextc then
        ...
        if cycle_filters then
          ...
          local get_cached_filter_result = function(f, a, b)
            b = b or false -- handle nilawful.key({ modkey,           }, "Tab",
  ...
  capi.keygrabber.run(function(mod, key, event)
    ...
    if not wbox_screen or wbox_screen ~= initial_screen then
      ...
      local w = math.ceil(wa.width * 0.4)
      wbox:geometry({
        x = wa.x,
        width = w,
      })
    ...
    local w = container_margin_left_right * 2
    wbox:geometry({
      height = h,
